import React from 'react';

export default function About() {
    return (
        <section id="about" className="py-16 px-10 scroll-mt-22">
            <h1 className="text-6xl md:text-9xl mb-8 text-center">About Me</h1>
            <p className="text-lg text-justify md:text-2xl max-w-6xl mx-auto">
                Hey there, my name is Gavin! I&apos;m a graduate from the University of California Santa Cruz with a Bachelor&apos;s in Computer Science. My 
                experience as a developer primarily has come from developing full-stack web applications, contributing to the frontend and backend. 
                I&apos;m currently at SpaceLab, volunteering as a front end developer providing my experience and improving my skills as a developer.
            </p>
            <p className="text-lg text-justify md:text-2xl max-w-6xl mx-auto">
                Right now, I&apos;m actively searching for a software developer position where I can grow my skillset and contribute to more projects. In 
                the meantime I&apos;m currently improving my skills as a developer and knowledge on the topics of AI and machine learning.
            </p>
        </section>
    );
}
